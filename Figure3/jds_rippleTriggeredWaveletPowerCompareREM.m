function jds_rippleTriggeredWaveletPowerCompareREM(animalprefixlist, days)
...
%   isolated (non-chain) cortical REM ripple onsets on the ctxriptet with
%   the most ripples, then compares mean power per frequency bin between
%   the two ripple types (t-test per bin, significant runs marked).
%
...
for l = 1:length(list)
    startidx = fBins(list{l}(1));
    endidx = fBins(list{l}(end));
    x = [startidx endidx];
