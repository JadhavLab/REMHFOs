%jds_thetaNestingSurrogate
%Theta nesting of gamma and ripple oscillations in PFC during phasic REM:
%finds each tetrode's preferred theta phase for gamma and ripple amplitude,
%extracts LFP around amplitude peaks within that phase bin, counts nested
%oscillation cycles, and compares peak-trough amplitudes against
%time-jittered surrogate windows. Also compares gamma vs ripple theta
%phase preference and nested-oscillation frequency.
            ...
            for t = 1:length(tetlist)
                ...
                if size(phaseIdxG,1) > 20
                    ...
                    for p = 1:length(pklocs)
                        ...
                        tmpdata = [tmpdata; abs(gamWin(pklocs(p))*0.195-gamWin(trlocs(tmp))*0.195)];
                    end
