function jds_remRippleCA1CoactivitySpatialCorr(animalprefixlist)
%JDS_REMRIPPLECA1COACTIVITYSPATIALCORR Spatial correlation vs REM ripple cofiring.
%   jds_remRippleCA1CoactivitySpatialCorr(animalprefixlist) computes, for
%   CA1 cell pairs tracked from the preceding run epoch, the correlation
%   between their place-field maps and their cofiring within REM chain
%   cortical ripples, and compares spatial correlations for positively vs
%   negatively cofiring pairs (rank-sum, boxplot).
%
...
figure;
h = boxplot(datacombinedSpCorr,g,'OutlierSize',7,'Symbol','k+'); set(h(7,:),'Visible','off');
ylabel('Spatial correlation (r)')
title(['CA1 spatial correlation by REM ripple cofiring-p = ' num2str(p1)])
